...
(function (doc, Routing, eZ, Translator, React, ReactDOM) {
  ...
  var onConfirm = function onConfirm(event, items) {
    ...
    items.forEach(function (item) {
      var numberOfItems = editorialModelsContainer.querySelectorAll(SELECTOR_EDITORIAL_ITEM).length;
      ...
      if (!alreadySelectedItem) {
        editorialModelsContainer.insertAdjacentHTML('beforeend', template.replace('__label_name__', contentName).replaceAll('__name__', numberOfItems));
        var lastItemNode = editorialModelsContainer.querySelector("".concat(SELECTOR_EDITORIAL_ITEM, ":last-child"));
        lastItemNode.querySelector('.ez-editorial-models__input--id').value = contentId;
        lastItemNode.querySelector('.ez-editorial-models__input--type').value = contentTypeId;
        lastItemNode.querySelector(SELECTOR_REMOVE_GROUP_ITEM_BUTTON).addEventListener('click', removeGroupItem, false);
      }
    });
    updateEditorialContainer();
  };
...
module.exports = __webpack_require__(/*! /home/jenkins/workspace/DXP/Archive_Maker_Commerce/ibexa-commerce/vendor/ezsystems/ezplatform-personalization/src/bundle/Resources/public/js/model.edit.js */"./vendor/ezsystems/ezplatform-personalization/src/bundle/Resources/public/js/model.edit.js");
